...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    if (state.phase === "projects") {
      ...
      // Publish per-project coverage after EVERY indexing chunk (not just at the
      // end) so Coverage fills in progressively and survives a partial or
      // interrupted crawl. Merge into the existing result to keep the leaderboard
      // and hot-file stats intact.
